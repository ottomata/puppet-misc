...
#
# You must have Maatkit installed: http://www.maatkit.org/
#
define mysql_user(
  $ensure       = "present",
...
# Sets up and manages a mysql multi instance, managed using supervisord.
#
define mysql_instance (
  $version = '5.0.92',
...
# installs the maatkit mysql tools
# http://www.maatkit.org/tools.html

class maatkit_package {
  package { "maatkit":
    ensure => "installed",
  }
}
...
# Installs the mylvmbackup rpm
class mylvmbackup_package {
  package { ["perl-DBD-MySQL", "mylvmbackup-0.13"]:
    ensure  => "installed",
  }
}
